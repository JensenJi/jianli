...
async function requireAdmin(c) {
  ...
  if (!authHeader?.startsWith("Bearer "))
    return false;
  try {
    const payload = await verify2(authHeader.slice(7), c.env.JWT_SECRET);
...
__name(getVisitorInfo, "getVisitorInfo");
app.post("/api/auth/register", async (c) => {
  ...
  const token = await sign2({ email, username }, c.env.JWT_SECRET, "HS256");
...
app.post("/api/auth/login", async (c) => {
  ...
  if (!await verifyPassword(password, user.password))
    return c.json({ error: "\u90AE\u7BB1\u6216\u5BC6\u7801\u9519\u8BEF" }, 401);
  const token = await sign2({ email, username: user.username }, c.env.JWT_SECRET, "HS256");
...
app.post("/api/messages", async (c) => {
  ...
  let user;
  try {
    user = await verify2(authHeader.slice(7), c.env.JWT_SECRET);
  } catch {
    return c.json({ error: "\u767B\u5F55\u5DF2\u8FC7\u671F" }, 401);
  }
...
app.put("/api/messages/:id/review", async (c) => {
  ...
  message.reviewedAt = (/* @__PURE__ */ new Date()).toISOString();
...
app.post("/api/visit", async (c) => {
  ...
  const isNew = !visited;
  if (isNew) {
    await c.env.VISITORS.put(visitorKey, "1", { expirationTtl: 86400 * 30 });
  }
  const pvId = crypto.randomUUID();
  const pv = { id: pvId, page: page || "/", visitor, timestamp: (/* @__PURE__ */ new Date()).toISOString() };
